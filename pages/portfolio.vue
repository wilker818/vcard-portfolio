      ...
      <div class="filter-select-box">
        <button :class="{ active: filterMenu }" class="filter-select" @click="filterMenu = !filterMenu">
          <div class="select-value">
            {{ activeCategory !== 0 ? (locale === 'en' ? activeCategoryName?.en : activeCategoryName?.pt) : 'Select Category' }}
          </div>
      ...
      <ul class="project-list">
        <li v-for="project in projectList" :key="project.id" :class="{ active: activeCategory === project.category.id || activeCategory === 0 }" class="project-item">
          <a class="cursor-pointer" @click="showItem(project.id)">
            ...
            <p class="project-category">{{ locale === 'en' ? project.category.title?.en : project.category.title?.pt }}</p>
        ...
        <section class="testimonials-modal block">
          ...
          <div class="modal-content space-y-3 mt-4">
            ...
            <small class="flex items-center justify-start gap-2 text-gray-500">
              <span>20 May, 2023</span> | <span>{{ locale === 'en' ? activeItem.category?.title?.en : activeItem.category?.title?.pt }}</span> | <a :href="activeItem.linkToProject" target="_blank">View Project</a>
            </small>

            <p class="text-justify" v-html="locale === 'en' ? activeItem.content?.en : activeItem.content?.pt" />
          </div>
        </section>
      </div>
    </section>
